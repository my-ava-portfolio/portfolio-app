import {
    faAppStore, faGithub, faLinkedinIn, faPython, faYoutube
} from "@fortawesome/free-brands-svg-icons";

import { faArrowAltCircleUp } from "@fortawesome/free-regular-svg-icons";
import {
    faHome,
    faUserGraduate,
    faGlobeEurope,
    faAddressCard,
    faBuilding,
    faStar,
    faMap,
    faImages,
    faFileAlt,
    faLanguage,
    faMapMarkerAlt,
    faBook,
    faFilePdf,
    faAngleDoubleDown,
    faFilter,
    faGlobe,
    faPaintBrush,
    faTrophy,
    faTag,
    faChartBar,
    faProjectDiagram,
    faTools,
    faEnvelope,
    faPhone,
    faBars,
    faCogs,
    faAlignJustify,
    faAlignLeft,
    faBug,
    faExclamationCircle,
    faQuestionCircle,
    faMobileAlt
} from "@fortawesome/free-solid-svg-icons";


// globals
export const mobileIcon = faMobileAlt;
export const arrowUpIcon = faArrowAltCircleUp;

// portoflio topics
export const homeIcon = faHome;
export const graduateIcon = faUserGraduate;
export const resumeIcon = faAddressCard;
export const navigationIcon = faGlobeEurope;
export const companyIcon = faBuilding;
export const mapIcon = faMap;

// left navigation
export const stackIcon = faCogs;
export const menuIcon = faBars;
export const bugIcon = faBug;
export const helpIcon = faQuestionCircle;
export const exclamationIcon = faExclamationCircle

// top navigation
export const navBarIcon = faAlignLeft;
export const subMenuIcon = faAlignJustify;

// common
export const locationIcon = faMapMarkerAlt;
export const galleryIcon = faImages;
export const blogIcon = faFileAlt;
export const skillIcon = faStar;
export const githubIcon = faGithub;
export const pdfFileIcon = faFilePdf;


// resume
export const linkedinIcon = faLinkedinIn;
export const emailIcon = faEnvelope;
export const phoneIcon = faPhone;
export const websiteIcon = faGlobe;

// education
export const languageIcon = faLanguage;
export const degreeIcon = faUserGraduate;
export const publicationIcon = faBook;


// experiences
export const filterIcon = faFilter;
export const trophyIcon = faTrophy;
export const arrowsDownIcon = faAngleDoubleDown;
export const projectIcon = faPaintBrush;
export const webSiteIcon = faGlobe;

// gallery
export const tagIcon = faTag;
export const chartIcon = faChartBar;
export const mapItemIcon = faMap;
export const appIcon = faAppStore;
export const toolIcon = faTools;
export const libraryIcon = faPython;  // TODO improve it
export const SpecificationIcon = faProjectDiagram;
export const videoIcon = faYoutube;
